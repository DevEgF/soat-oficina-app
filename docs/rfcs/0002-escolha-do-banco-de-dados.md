...
O projeto Neon foi criado pelo console; **não foi provisionado pelo módulo Terraform RDS**. Esse módulo continua sendo a entrega IaC histórica do banco AWS. Credenciais S3/AI Gateway não são credenciais PostgreSQL. [Operação, justificativas e evidências](https://github.com/DevEgF/soat-oficina-app/blob/develop/README.md).
