...
HTTPS HML/PROD, sete migrations por schema, saúde/login/leitura em produção, jornada de OS em hml, rejeição de JWT cruzado, dashboards e dois incidentes de autenticação foram verificados. Rollback OCI validou configuração Helm e preservação de OS com os mesmos digests. [Links e matriz operacional](https://github.com/DevEgF/soat-oficina-app/blob/develop/README.md).
